...
- [x] 0. Apply PR #26 hardening and resolve issue #22 lint/type fixes
  - [x] 0.1 Harden `apps/dashboard/src/app/api/intents/route.ts` with explicit types from PR #26
  ...
  - [x] 0.2 Fix `apps/dashboard/src/lib/store.ts` lint/type issues
  ...
  - [x] 0.3 Fix `apps/dashboard/src/components/providers.tsx` lint issues
    - Replace `{ children: any }` with `{ children: React.ReactNode }` in `Providers` component
    - _References: [GitHub Issue #22](https://github.com/hashpass-tech/JACK/issues/22)_
  - [x] 0.4 Run `pnpm --filter dashboard lint` and `pnpm --filter dashboard build` to verify all fixes
    - _References: [GitHub Issue #22](https://github.com/hashpass-tech/JACK/issues/22)_

- [x] 1. Install `@lifi/sdk` dependency and create LI.FI types
  - [x] 1.1 Add `@lifi/sdk` as a runtime dependency in `packages/sdk/package.json` and run `pnpm install`
    - _Requirements: 1.1_
  - [x] 1.2 Create `packages/sdk/src/lifi/types.ts` with `LifiProviderType`, `LifiReasonCode`, `LifiFallback`, `LifiQuotePayload`, `LifiRoutePayload`, `LifiStatusPayload` types
    - Define all type definitions as specified in the design document
    - _Requirements: 3.2, 4.2, 7.2_

- [x] 2. Implement ChainMap and TokenMap pure modules
  - [x] 2.1 Create `packages/sdk/src/lifi/chain-map.ts` with `resolveChain` and `getSupportedChains`
  ...
  - [x] 2.3 Create `packages/sdk/src/lifi/token-map.ts` with `resolveToken` and `getSupportedTokens`
...
- [x] 3. Implement unit conversion utilities and fallback provider
  - [x] 3.1 Create `packages/sdk/src/lifi/utils.ts` with `toBaseUnits` and `fromBaseUnits`
  ...
  - [x] 3.3 Create `packages/sdk/src/lifi/fallback.ts` with `buildFallbackQuote`, `buildFallbackRoute`, `buildFallbackStatus`, and `deterministicId`
...
- [x] 5. Implement LifiProvider core
  - [x] 5.1 Create `packages/sdk/src/lifi/lifi-provider.ts` with `LifiConfig` interface and `LifiProvider` class
  ...
  - [x] 5.2 Implement `fetchQuote` method on LifiProvider
  ...
  - [x] 5.3 Implement `fetchRoute` method on LifiProvider
  ...
  - [x] 5.4 Implement `fetchStatus` method on LifiProvider
...
- [x] 7. Wire LifiProvider into JACK_SDK and add barrel exports
  - [x] 7.1 Create `packages/sdk/src/lifi/index.ts` barrel export for the lifi submodule
    ...
    - _Requirements: 1.6_
  - [x] 7.2 Update `packages/sdk/src/index.ts` to re-export from `./lifi/index.js`
    - Add LifiProvider, LifiConfig, and all LI.FI types to the SDK barrel export
    - _Requirements: 1.6_
  - [x] 7.3 Extend `JACK_SDK` class to accept optional `lifi` config and expose `LifiProvider` instance
...
- [x] 9. Migrate dashboard to use SDK-level LifiProvider
  - [x] 9.1 Refactor `apps/dashboard/src/lib/lifi.ts` to import from `@jack-kernel/sdk`
