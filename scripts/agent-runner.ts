  ...
  async executeTask(task: Task): Promise<TaskResult> {
    ...
    return {
      ...
      verification: {
        compiled: true,
        tests_passed: true,
        acceptance_met: task.acceptance ?? [],
      },
...
Press ENTER when done...
    ...
    return {
      ...
      verification: {
        compiled: exists,
        tests_passed: exists,
        acceptance_met: exists ? (task.acceptance ?? []) : [],
      },
...
export class CodexAdapter implements AgentBackend {
  ...
  async executeTask(task: Task): Promise<TaskResult> {
    ...
    const autoExecute =
      process.env.CODEX_AUTO_EXEC === "1" || !isInteractiveTty;
...
export class TaskOrchestrator {
  ...
  async executeTask(
    ...
    const agent = await this.selectAgent(
      task,
      options?.preferredAgent ?? task.agent_config?.preferred,
    );
  ...
  private async verifyAndCommit(task: Task, result: TaskResult) {
    ...
    try {
      if (!result.output_path) {
        console.warn(
          `⚠️ No output_path for task ${task.id}; skipping git commit.`,
        );
        return;
      }
      ...
      execSync(
        `git commit -m "feat: ${task.title} (agent: ${result.agent_log})${closeMsg}"`,
        { stdio: "ignore" },
      );
    } catch (e) {
...
function parseArgs(argv: string[]) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "--help" || arg === "-h") {
      console.log(
        `Usage: pnpm agent:run [taskFile] [--task <id>] [--agent <name>] [--verify] [--no-commit]`,
      );
      process.exit(0);
    }
...
async function main() {
  ...
  const { filePath, taskId, preferredAgent, verify, noCommit } = parseArgs(
