...
Press ENTER when done...
    `);
        ...
        const exists = fs.existsSync(task.output.path);
        return {
            success: exists,
            output_path: task.output.path,
...
export class TaskOrchestrator {
    ...
    async executeTask(task: Task, preferredAgent?: string): Promise<TaskResult> {
        const agent = await this.selectAgent(task, preferredAgent);
    ...
    private async selectAgent(task: Task, preferred?: string): Promise<AgentBackend> {
        ...
        return new CursorAdapter(); // Fallback to manual
    }
...
async function main() {
    ...
    const filePath = process.argv[2] || '.agent-tasks/tasks.yaml';
    ...
    for (const task of tasksData.tasks) {
        const result = await orchestrator.executeTask(task, process.env.PREFERRED_AGENT);
        if (!result.success) {
