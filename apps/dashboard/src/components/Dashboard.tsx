...
const ThemeToggle: FC = () => {
  const [theme, setTheme] = useState<"dark" | "light">(() => {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("jack-theme") as
        | "dark"
        | "light"
        | null;
      return stored || "dark";
  ...
  return (
    ...
    >
      {theme === "dark" ? (
        <svg
          className="w-4 h-4 text-[var(--fg-accent)]"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
...
const OnboardingVideoModal: FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const toggleFullscreen = () => {
    ...
    if (!document.fullscreenElement) {
      el.requestFullscreen?.()
        ...
        .catch(() => {
          // Fallback: just use CSS fullscreen if API blocked
  ...
  // Keyboard shortcut: F for fullscreen, Escape to exit fullscreen or close
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="relative"
        style={{
          width: isFullscreen ? "100vw" : "min(93vw, 93vh * 16 / 9)",
          height: isFullscreen ? "100vh" : "min(93vh, 93vw * 9 / 16)",
          maxWidth: isFullscreen ? "100vw" : undefined,
          maxHeight: isFullscreen ? "100vh" : undefined,
          borderRadius: isFullscreen ? 0 : "clamp(0.75rem, 2vw, 1.5rem)",
          overflow: "hidden",
          background: "#000",
          boxShadow: isFullscreen
            ? "none"
            : "0 0 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.06)",
      ...
      >
        ...
        >
          ...
          />
          <span className="text-[8px] sm:text-[10px] font-bold uppercase tracking-[0.2em] text-white/70">
            JACK Walkthrough
          </span>
        </div>
        ...
        <div
          ...
          style={{
            ...
            padding:
              "clamp(2rem, 6vw, 3.5rem) clamp(0.75rem, 2vw, 1.25rem) clamp(0.75rem, 2vw, 1.25rem)",
          }}
        >
          {/* Left: transport */}
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <button
              ...
              className="flex w-8 h-8 sm:w-9 sm:h-9 rounded-full items-center justify-center transition-all hover:scale-110 active:scale-95"
              style={glass}
              ...
              {isFullscreen ? (
                ...
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M8 4H4v4m16-4h-4v0h0V4m0 16v-4h4m-16 4v-4H4"
                  />
                </svg>
              ) : (
                <svg
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <button
              ...
              onClick={() => {
                localStorage.setItem("jack-hide-onboarding", "true");
                onClose();
              }}
            >
              Don&apos;t show again
    ...
    >
      <div
        ...
        style={{
          ...
          padding: "1.75rem",
          boxShadow:
            "0 20px 60px rgba(0,0,0,0.4), 0 0 30px rgba(56,189,248,0.05)",
          opacity: entered ? 1 : 0,
          transform: entered
...
const Dashboard: FC<{ changelog?: string }> = ({ changelog = "" }) => {
  ...
  const tabs: {
    key: "create" | "executions" | "cost-dashboard";
    label: string;
    shortLabel: string;
  }[] = [
    ...
    >
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                ...
                <p
                  className="text-xs font-bold tracking-widest leading-none"
                  style={{ color: "var(--fg-primary)" }}
                >
                  Kernel
                </p>
            ...
            >
              {tabs.map((tab) => (
                <button
                  ...
                  style={
                    (activeTab === tab.key && !selectedExecutionId) ||
                    (tab.key === "executions" && selectedExecutionId)
                      ? {
                          background: "var(--fg-accent)",
          ...
          <div className="flex items-center space-x-2 md:hidden">
            ...
            >
              <div
                className={`w-5 h-0.5 transition-all duration-300 ${mobileMenuOpen ? "rotate-45 translate-y-2" : ""}`}
                style={{ background: "var(--fg-accent)" }}
              />
              <div
                className={`w-5 h-0.5 transition-all duration-300 ${mobileMenuOpen ? "opacity-0" : ""}`}
                style={{ background: "var(--fg-accent)" }}
              />
              <div
                className={`w-5 h-0.5 transition-all duration-300 ${mobileMenuOpen ? "-rotate-45 -translate-y-2" : ""}`}
      ...
      {isTestnet && showBanner && (
        ...
        >
          ...
          >
            <span
              className="text-[11px] font-bold mx-8 leading-[32px]"
              style={{ color: "var(--bg-primary)" }}
            >
              ⚠️ TESTNET MODE · For mainnet, visit{" "}
              <a
                href="https://jack.lukas.money/dashboard"
                className="underline font-black hover:opacity-80"
                target="_blank"
        ...
        >
          <div
            ...
            style={{
              background: "var(--bg-secondary)",
              borderColor: "var(--border-accent)",
              boxShadow: `0 0 50px var(--shadow-accent)`,
            }}
          >
